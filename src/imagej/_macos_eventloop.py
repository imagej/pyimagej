...
class EventLoopConsole(code.InteractiveConsole):
    def __init__(self, *args, **kwargs):
        super().__init__(*args, **kwargs)
        self._init_console_input_watcher()
    ...
    def _detect_console_input(self):
        """Check if anything as been inputed into the console."""
        while True:
            readable = select.select([sys.stdin], [], [], 0)[0]
            if not readable:
                pass
            else:
                # input detected, stop the loop
                global stop_loop
                stop_loop = True

    def _init_console_input_watcher(self):
        Thread(target=self._detect_console_input, daemon=True).start()

    def _eventloop(self):
        global stop_loop
        while not stop_loop:
            self.runcode("cocoa_mainloop()")
        else:
            line = input(">>> ")
            stop_loop = False
            self.push(line)
